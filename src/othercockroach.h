#ifndef OTHER_COCKROACH_H
#define OTHER_COCKROACH_H

#include "enemy.h"
#include <array>

class OtherCockroach : public Enemy
{
    
public:

 //constructor to set up resources
    OtherCockroach( int x, int y,int width,int height);

    //destructor to free resources
    virtual ~OtherCockroach();
...
//State
    ...
    //state specific to cock roach
    enum class CockroachState : std::int8_t {MOVE_UP = 0,MOVE_DOWN,MOVE_LEFT,MOVE_RIGHT,
                                            PAUSE,
                                            DETERMINE_DIRECTION};
...
private:
    ...
    std::stack <OtherCockroach::CockroachState> state_stack;
    
    void pushState(OtherCockroach::CockroachState state);
    void popState();
    OtherCockroach::CockroachState getCurrentState();
